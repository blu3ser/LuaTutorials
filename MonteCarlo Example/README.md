...
2. Second and following Monte Carlo runs:
   - Open the Lua Console and type ITERATION = 0
...
1. **CommandCLI (Automatic):**
   ```
   /path/to/CommandCLI.exe -mode mc -scenario /path/to/scenario.scen -it 60  → ITERATION: 1-60
   ```
...
**Pros:** Clean separation, easier to debug, can test the base scenario independently, recommended for beginners
...
**Pros:** Complete control, everything in code, easy to version control and share, recommended for experienced users
...
**Pros:** Recommended for intermediate users, or if you want to modify an existing scenario
...
```lua
if ITERATION <= 10 then
    loadoutid = 12345  -- Weapon Y loadout
else
    loadoutid = 67890  -- Weapon X loadout
end
```
...
3. **UI vs CommandCLI Difference**:
   ...
   - **UI**: Does NOT reset - you must manually reset ITERATION = 0 before each new Monte Carlo run
...
6. **Conditional Logic**: Simple `if/then` statements control what gets added/modified based on iteration number. Create a settings function like example to a more advanced configuration
...
- Open the lua console and run `ITERATION = 0` then you can launch your Montecarlo run
